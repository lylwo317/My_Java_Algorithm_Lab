...
public class RBTree<T extends Comparable<T>> implements BinaryTreeInfo {
    ...
    public boolean isRBTree() {
        ...
        RBTreeNode<T> calHeightNode = root;
        while (calHeightNode != null) {
            if (isBlack(calHeightNode) && calHeightNode != root) {
                rootNodeBlackHeight++;
            }
            calHeightNode = calHeightNode.getLeft();
        }
        rootNodeBlackHeight++;

        Deque<RBTreeNode<T>> nodeStack = new ArrayDeque<>();
        do {
            while (node != null) {
                if (!(node.getLeft() != null && node.getRight() != null) && node != root) {
                    RBTreeNode<T> iteratorNode = node;
                    int blackCount = 0;
                    boolean preColorIsRed = false;
                    while (iteratorNode != null && iteratorNode != root) {
                        if (isBlack(iteratorNode)) {
                            blackCount++;
                        } else {
                            if (preColorIsRed) {//不符合性质(4)（不能有两个连续的红色节点）
                                throw new IllegalStateException("This is not a red-black tree. Because has continues reb node at " + node);
                            }

                        }
                        preColorIsRed = iteratorNode.isRed();
                        iteratorNode = iteratorNode.getParent();
                    }
                    blackCount++;

                    if (blackCount != rootNodeBlackHeight) {//不符合(5)
            ...
            if (!nodeStack.isEmpty()) {
                RBTreeNode<T> pop = nodeStack.pop();
                node = pop.getRight();
            }
    ...
    public Object left(Object node) {
        return ((RBTreeNode) node).getLeft();
    }

    @Override
    public Object right(Object node) {
        return ((RBTreeNode) node).getRight();
    }
