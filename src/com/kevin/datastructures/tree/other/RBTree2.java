...
public class RBTree2<T extends Comparable<T>> implements BinaryTreeInfo {
    ...
    public Object left(Object node) {
        return ((RBTNode<T>) node).left;
    }

    @Override
    public Object right(Object node) {
        return ((RBTNode<T>) node).right;
    }
    ...
    public void checkRBTreeProperties() {
        if (!isBlack(mRoot)) {//不符合性质(1)
            throw new IllegalStateException("root node color isn't black. root = " + mRoot);
        }
        ...
        Queue<RBTNode<T>> nodeQueue = new LinkedList<>();
        nodeQueue.offer(mRoot);
        while (!nodeQueue.isEmpty()) {

            RBTNode<T> node = nodeQueue.poll();

            if (node.left == null || node.right == null) {//计算度为1或者度为0的节点到根节点的黑色节点数量是否等于rootNodeBlackHeight，以及是否存在两个连续红色节点
                RBTNode<T> iteratorNode = node;
                int blackCount = 0;
                boolean preColor = BLACK;
                while (iteratorNode != null) {//往根节点遍历，看是否存在连续两个红色节点
                    if (isBlack(iteratorNode)) {
                        blackCount++;//统计黑色节点数量
                    } else {//当前遍历的节点是红色
                        if (preColor == RED) {//前一个节点也是红色，不满足性质(4)
